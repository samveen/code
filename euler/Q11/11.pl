#!/usr/bin/perl
use strict;
my @src = ( [qw(08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08)],
...
my $height=scalar(@src);
my $length=scalar(@{$src[0]});

my ($max,$x,$y,$dir,$temp)= (-1,-1,-1,"",0);

# Start: Top Left. Direction of index increase: Down, Left, Bottom Left, Bottom Right
for(my $i=0; $i<$height; ++$i) {
    for(my $j=0; $j<$length; ++$j) {
        if ($j<$length-4) { # RIGHT
            $temp=$src[$i][$j]*$src[$i][$j+1]*$src[$i][$j+2]*$src[$i][$j+3];
            ($max,$x,$y,$dir) = ($temp,$i,$j,"right") if $temp > $max;
        }
        if ($i<$height-4) { # DOWN
            $temp=$src[$i][$j]*$src[$i+1][$j]*$src[$i+2][$j]*$src[$i+3][$j];
            ($max,$x,$y,$dir) = ($temp,$i,$j,"down") if $temp > $max;
        }
        if($j<$length-4 and $i<$height-4) { # DOWN RIGHT
            $temp=$src[$i][$j]*$src[$i+1][$j+1]*$src[$i+2][$j+2]*$src[$i+3][$j+3];
            ($max,$x,$y,$dir) = ($temp,$i,$j,"dr_diag") if $temp > $max;
        }
        if($j>=4 and $i<$height-4) { # DOWN LEFT
            $temp=$src[$i][$j]*$src[$i+1][$j-1]*$src[$i+2][$j-2]*$src[$i+3][$j-3];
            ($max,$x,$y,$dir) = ($temp,$i,$j,"dl_diag") if $temp > $max;
        }
    }
}
print "$max starting at $x,$y going $dir\n";
